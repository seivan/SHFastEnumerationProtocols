...
-(void)testToArray; {
  ...
  XCTAssertTrue([self.matching isKindOfClass:[NSArray class]]);
  XCTAssertEqualObjects(self.subject, self.matching);
}
...
-(void)testReverse; {
  ...
  XCTAssertEqualObjects(self.subject.reverseObjectEnumerator.allObjects, self.matching);
  XCTAssertTrue(self.matching.count > 0);
...
-(void)testModifyMap; {
  ...
  XCTAssertTrue(self.matching.count < self.subject.count);
  XCTAssertTrue(self.matching.count == 1);
